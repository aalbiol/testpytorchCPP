...
void tensor_savecimgList(const torch::Tensor & in, std::string & fname){
   cimg_library::CImg<float> cblobresult(in.data_ptr<float>(), in.size(3), in.size(2), in.size(1), in.size(0));
   cimg_library::CImgList<float> clist = cblobresult.get_split('c');
   cimglist_for(clist, l) {
      clist[l]=clist[l].get_split('z').get_append('c');
   }
   clist.save(fname.c_str());
...
void forwardCList(const cimg_library::CImgList<float>& clist, const std::vector<float>& mean, const std::vector<float>& std, torch::jit::script::Module& model, torch::Device& device, float scale_factor, cv::Size im_sz, torch::Tensor& out_tensor) {
   ...
   std::string fname="input_tensor.cimg";
   tensor_savecimgList(batch_tensor,fname);
