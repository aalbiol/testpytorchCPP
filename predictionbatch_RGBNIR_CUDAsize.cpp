...
int main(int argc, const char* argv[]) {
   ...
   tamanyos.push_back(110);
   tamanyos.push_back(100);
   tamanyos.push_back(90);
   tamanyos.push_back(80);
   tamanyos.push_back(70);

   std::vector<int> tiempos(tamanyos.size());
   for (int n = 0; n < tamanyos.size(); n++) {
      int tam = tamanyos[n];
      cv::Size im_sz(tam, tam);
   ...
   for (int n = 0; n < tamanyos.size(); n++) {
      std::cout << "Tam:" << tamanyos[n] << " ---> tiempo: " << tiempos[n] << "\n";
   }
}
