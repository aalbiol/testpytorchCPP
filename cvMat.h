      ...
      switch (src.depth()) {
         ...
         case CV_8U:  // 8-bit unsigned int.
            if (src.channels() == 1) {
               ...
               CImg<ucharT> tmp(src.cols, src.rows, 1, nchannels);
               for (int c = 0; c < nchannels; ++c) {
                  std::memcpy(tmp.get_shared_channel(c).data(), channels[c].ptr(), src.cols * src.rows * sizeof(uchar));
               }
               tmp.move_to(*this);
            }
            break;
         ...
         case CV_8S:  // 8-bit signed int.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<ucharT> tmp(src.cols, src.rows, 1, 3), R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
         ...
         case CV_16U:  // 16-bit unsigned int.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<ushortT> tmp(src.cols, src.rows, 1, 3), R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
         ...
         case CV_16S:  // 16-bit signed int.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<shortT> tmp(src.cols, src.rows, 1, 3), R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
         ...
         case CV_32S:  // 32-bit signed int.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<intT> tmp(src.cols, src.rows, 1, 3), R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
         ...
         case CV_32F:  // 32-bit float.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<floatT> tmp(src.cols, src.rows, 1, nchannels);
               for (int c = 0; c < nchannels; ++c) {
                  std::memcpy(tmp.get_shared_channel(c).data(), channels[c].ptr(), src.cols * src.rows * sizeof(float));
               }
               tmp.move_to(*this);
            }
            break;
         case CV_64F:  // 64-bit double.
            if (src.channels() == 1) {
               ...
               std::vector<cv::Mat> channels;
               cv::split(src, channels);
               CImg<doubleT> tmp(src.cols, src.rows, 1, 3), R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            break;
         default:
            throw CImgInstanceException(_cimg_instance "assign(const cv::Mat&) : Mat depth is invalid.", cimg_instance);
            break;
      }
   } else {
      ...
      switch (src.depth()) {
         ...
         case CV_32F:  // 32-bit float.float
            if (src.channels() == 1) {
               ...
               cv::split(src, channels);
               for (int c = 0; c < src.channels(); ++c) {
                  CImg<floatT> plane = tmp.get_shared_channel(c);
                  for (int i = 0; i < size.height; ++i) {
...
// Conversion CImg -> MAT // devuelve BGR
cv::Mat get_MAT(const unsigned int z = 0) const {
   ...
   if (z >= _depth) throw CImgInstanceException(_cimg_instance "get_MAT() : specified slice %u is out of image bounds.", cimg_instance, z);
   ...
   if (matType < 0) throw CImgInstanceException(_cimg_instance "get_MAT() : pixel type '%s' is not supported.", cimg_instance, buf.pixel_type());
   cv::Mat out;
...
// Similar pero el orden de los canales no se cambia. Devuelve RGB o RGBNIR
cv::Mat get_MAT2() const {
   if (is_empty()) throw CImgInstanceException(_cimg_instance "get_MAT() : instance image is empty.", cimg_instance);

   const CImg<T>& buf = *this;
   int cols = buf.width(), rows = buf.height(), nchannels = buf.spectrum(), matType = -1;
   ...
   if (nchannels > 1) {
      for (int c = 0; c < nchannels; ++c) {
         channels[c] = cv::Mat(rows, cols, matType, const_cast<T*>(buf.data() + rows * cols * c));
      }  // for channels
      cv::merge(channels, out);
   } else
      out = cv::Mat(rows, cols, matType, const_cast<T*>(buf.data())).clone();
