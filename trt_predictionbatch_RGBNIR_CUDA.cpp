...
int main(int argc, const char* argv[]) {
   ...
   std::cerr << "comienzo a optimizar el modelo para torch_tensorrt...\n";

   model.eval();

   auto inp = std::vector<int64_t>{ 8,4,110,110 };
   auto input = torch_tensorrt::Input(inp);
   
   
   auto compile_settings = torch_tensorrt::ts::CompileSpec({ input });
   compile_settings.enabled_precisions.insert(torch::kFloat16);

   auto trt_model = torch_tensorrt::ts::compile(model, compile_settings);

   trt_model.eval();
   std::cout << "Modelo optimizado\n";
   ...
   for (int r = 0; 1; r++) {
      ...
      forwardCList(listimg, mean, std, trt_model, device, scale_factor, im_sz, out_tensor);
      int64 t1 = cv::getTickCount();
